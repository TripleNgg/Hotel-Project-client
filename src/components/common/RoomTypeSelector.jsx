import React, { useEffect, useState } from "react";
import PropTypes from 'prop-types';
import { getRoomTypes } from "../utils/ApiFunctions";
import { da } from "date-fns/locale";

const RoomTypeSelector = (props) => {
  const { handleRoomInputChange, newRoom } = props;
  const [roomTypes, setRoomTypes] = useState([""]);
  const [showNewRoomTypeInput, setShowNewRoomTypeInput] = useState(false);
  const [newRoomType, setNewRoomType] = useState("");
  const [messageError, setErrorMessage] = useState("");

  useEffect(() => {
    getRoomTypes().then((data) => {
      console.log(data);
      if(data.length == 0){
        setRoomTypes("single bed");
      }
      setRoomTypes(data);
    });
  }, []);

  const handleNewRoomTypeInputChange = (e) => {
    setNewRoomType(e.target.value);
  };

  const handleUnSelectedAddNew = () =>{
    setShowNewRoomTypeInput(false);
  }


  const handleAddNewRoomType = () => {
    if (newRoomType !== "") {
      setRoomTypes([...roomTypes, newRoomType]);
      setNewRoomType("");
      setErrorMessage("")
      setShowNewRoomTypeInput(false);
    }
  };

  return (
    <>
      {messageError && (
        <div className='alert alert-danger fade show'>
          {messageError}
        </div>
      )}
      {roomTypes.length > 0 && (
        <div>
          <select
            required
            className="form-select"
            name="roomType"
            onChange={(e) => {
              if (e.target.value === "Add New") {
                setShowNewRoomTypeInput(true);
              } else if (e.target.value === "") {
                handleUnSelectedAddNew();
                setErrorMessage("Please choose one room type");
              } else {
                handleRoomInputChange(e);
                handleUnSelectedAddNew();
              }
            }}
            value={newRoom.roomType}>
            <option value="">Select a room type</option>
            <option value={"Add New"}>Add New</option>
            {roomTypes.map((type, index) => (
              <option key={index} value={type}>
                {type}
              </option>
            ))}
          </select>
          {showNewRoomTypeInput && (
            <div className="mt-2">
              <div className="input-group">
                <input
                  type="text"
                  className="form-control"
                  placeholder="Enter New Room Type"
                  value={newRoomType}
                  onChange={handleNewRoomTypeInputChange}
                />
                <button
                  className="btn btn-hotel"
                  type="button"
                  onClick={handleAddNewRoomType}>
                  Add
                </button>
              </div>
            </div>
          )}
        </div>
      )}
    </>
  );
};

RoomTypeSelector.propTypes = {
  handleRoomInputChange: PropTypes.func.isRequired,
  newRoom: PropTypes.object.isRequired,
};

export default RoomTypeSelector;
